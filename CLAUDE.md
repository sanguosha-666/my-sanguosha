...
- **【已失效的历史记录】原先这里记着「`run_startgame_wiring_test.js` 场景4 有一个低概率随机性导致的 flaky」**——做 CORE-149（issue #208，主公候选池分层）时全仓库查证，**这个测试文件早已不存在**（`find . -name '*startgame*'` 无任何结果），当前碰 `startGame` 的测试是 `run_identity_mode_test.js`/`run_seat_shuffle_test.js`/`run_core149_lord_pool_test.js` 等。那条记录描述的候选切片碰撞场景（`debugPickGeneral` 绕开候选池、恰好与随机切片撞上）在当前代码里也不再成立：其他玩家的候选不是从 `shuffled` 顺序切的，而是主公选完后由 `finishLordGeneralPick` 从 `rest` 现发、已排除已选武将。**保留这条只为说明它已作废，避免以后又有人去找那个文件**；下次清理本节时可直接删除整条。
