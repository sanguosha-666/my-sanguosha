...
>
> - **历史改动的完整记录在 `docs/progress-log-*.md`**（按时间顺序分段，当前共 12 段，
>   `docs/progress-log-1.md` 最早、`docs/progress-log-12.md` 最新）。如果需要了解某个
...
> 1. 打开 `docs/` 目录下编号最大的 `progress-log-N.md`（目前是 `progress-log-12.md`）。
...
- **技能类型分类（主公技/限定技/觉醒技）走 cap 名单 seam**：三张名单 `LORD_SKILL_CAPS`/`LIMIT_SKILL_CAPS`/`AWAKEN_SKILL_CAPS`（`data.js`）登记"哪些 cap 属于该类技能"，派生查询 `generalHasLordSkill`（身份局主公候选池分层用）与 `huashenForbiddenCap`/`huashenEntryForbidden`（左慈【化身】不可借用主公技/限定技/觉醒技，官方规则）。**单一事实源始终是 `caps`**——不要在 `GENERALS` 上另加 `skillType`/`lordSkill` 之类字段，那等于把同一个事实记两处、迟早漂移（CORE-117 就是这类问题）。**新增武将时，若它带这三类技能中的任何一种，把对应 cap 加进相应名单即可**：化身表的收录校验（`validateHuashenPick`）会自动拒绝，不依赖"有没有人记得别往 `HUASHEN_SKILL_TABLE` 里加"。
...
- **确定正收益技能不问真人也不问 AI，直接自动生效**：binary"是否发动"且"不发动"完全没有意义（纯损失，不存在任何场景下"不发动更优"）的技能（如铁骑/烈弓/连营/闭月/礼让回收/酒诗翻正面/落英拾取/洛神判定），不要走"创建 `g.pending`、等待某人点击确认"这套标准响应流程——在原本"创建 pending 并 return"的位置，直接内联执行"发动=true"分支的核心逻辑，其余判定链路/续接逻辑（`maybeGuicai` 改判、`resume` 续接、`startTurn`/`advanceXiaoguo`/`continueDelayResolution` 等收尾）完全不变、原样保留（这些是**其他玩家**或**框架层**的独立决策/流程，不是这次要跳过的"发动者自己是否发动"这一层）。可能循环多轮的技能（如洛神黑色判定后继续判）改成一个独立的 `autoXxxRound(g,seat)` 函数，让"判定→结算→继续下一轮或结束"整个循环在一次调用里走完，不再每轮挂起单独的 pending。**代码清理粒度**：只需要清理会导致"引用已删除函数"这类真正悬空引用的部分（如果某个技能的循环结构决定了原 `respond*` 函数被整体删除）；如果原 `respond*`/`maybeStart*` 函数体保留未删（多数情况——函数继续存在只是不再有任何路径能触发到它），`render-controls.js`/`bot.js`/`stages/stage-table.js` 里对它的既有引用不会报错，是"安全的死代码"，可以留到后续单独任务批量清理，不必和这次改动本体绑在一起做。
...
### 人数
- `SEATS=9` 是**容量上限**（满 9 不再加入）；`MIN_PLAYERS=2` 是**开始门槛**（≥2 即可开始）。身份局另有自己的人数区间（`IDENTITY_TABLE` 覆盖 4~8 人）。
...
29. **完成任何改动后要跑多大范围的测试，按改动的风险等级判断，不是无差别每次都跑 `run_all_tests.js` 全量套件，也不是无差别只跑最小相关测试集。** 判断走两条独立的触发轴，任意一条命中就必须跑全量；两条都不命中才允许只跑最小相关测试集；信号矛盾/拿不准时，默认跑全量（宁可多跑，不要少跑）。

    **判断轴 A——改动是否修改了"被多处共用"的既有函数体**（新增函数不算，只看"改了已存在的函数逻辑"）：
    - 用 `grep` 在全仓库（`*.js`，尤其 `skills.js`/`weapons.js`/`sha/sha-resolution.js`/`bot.js`）搜这个函数名的调用点，超过一处调用、或被某张共享表（`CARD_PLAYS`/`EQUIPS`/`GENERALS`/`DELAY_TRICKS`/`STAGE_TABLE`/`BOT_DECISIONS`/`BOT_SEAT_PICKS`）引用 → 命中此轴，跑全量。
    - 典型真实案例：CORE-97/98/99/102（顺手牵羊/过河拆桥/借刀杀人/火攻的目标校验缺陷）——这几处改动看似只影响各自一张牌，实际都触碰了被多张牌共用的 `canTarget` 校验逻辑，若当初只跑局部测试就合并，其它复用同一段判断的牌不会被覆盖到。

    **判断轴 B——改动是否触碰下列"高连带风险子系统"（不看调用点数量，只要碰到就命中）**：
    - **机器人决策核心分发机制**：`bot.js`/`bot-ai-bus.js` 里的 `BOT_DECISIONS`/`BOT_SEAT_PICKS`/`BOT_PHASE_ACTOR`/`botDecide`/`runBotDecision`/`scheduleBotTurn`/`botSeatForState`/`botStateKey`——这条分发链任何一环的改动都可能影响全部技能的机器人决策，不是单一技能的局部改动（真实案例：CORE-113 的 `seatPickLocalFallback` 一处"取第一个matched技能"缺陷，影响面覆盖 `BOT_SEAT_PICKS` 表里全部技能；CORE-116 的 `botTwoStepA` 自我触发缺口，覆盖借刀/离间/丈八/仁德四个技能）。
    - **`pending` 状态流转/`normalize` 校验逻辑**：任何新增/修改 `g.pending` 的创建、清空、字段校验（尤其是 `normalize` 里对某个 `pending.type` 的校验分支）——这类改动历史上多次表现为"看似只影响一个技能，实际让整条状态机在别的技能/别的阶段卡死或被误判为脏数据清空"（真实案例：CORE-25 类"贾诩乱武/陈宫明策把合法中间态误判成脏数据"；CORE-112/113/114 那一串"链结束忘记显式清空 `g.pending`导致下一回合/下一技能被污染"）。
    - **`soak.js` 压测覆盖范围内的核心游戏逻辑**：`dealDamage`/`checkWin`/`canTarget`/`canUseAs`/`findUsableAs`/`distance`/`attackRange`/`hasCap`/`triggerHook`/`resolveShaUse`/`playCard`/`equipCard`/`resolveTrick` 这类贯穿全部技能与卡牌结算的公共入口，以及任何"轮流询问下一个候选人"（`nextAskee`/`nextGuicaiAsker`/`nextXiaoguoAsker` 这类）的遍历逻辑——这些函数不是任何单一技能专属，soak.js 的随机对局会自然把它们的改动放大到"任意技能组合下都可能触发"的范围，局部测试测不出组合态问题。
    - **`normalize(g)` 本身**、**`tx(fn)` 的封装逻辑**、**cache-bust 检查脚本（`check_cache_bust.js`）**、**测试聚合入口（`run_all_tests.js`）**——这几处是"每次改动都会经过"的基础设施，改坏了会让后续所有测试的可信度打折，必须用最严格的全量验证。
    - **`index.html` 里的脚本加载顺序**（`<script src=...>` 排列）——顺序错误可能导致某些函数在被调用时尚未定义，局部测试往往测不出（局部测试通常手工模拟加载顺序，不会自然复现真实浏览器的加载时序问题）。

    **可以只跑最小相关测试集的情形**（两条轴都不命中，且同时满足下列全部条件）：
    - 改动**只新增**函数/常量/DOM 元素，**不修改任何既有函数的函数体**（用 `git diff` 确认：变更行只出现在新增的代码块内，没有改动已存在函数内部的任何一行逻辑）。
    - 不触碰判断轴 B 列出的任何一个子系统/文件区域。
    - 不写入/不读取 `g`（共享房间状态）核心结算路径——纯客户端本地状态（`localStorage`/纯 UI 展示变量）或独立于 `tx()` 之外的渲染层新增。
    - 真实案例：CORE-115（身份猜测标记）——纯 UI 新功能，标记数据存 `localStorage`（不写入 `g`），新增的 3 个存储函数/1 个菜单函数/1 处 `renderSeatCard` 追加渲染逻辑全部是新增代码，未修改任何既有函数体，`newGame()` 里的改动是"新增一行调用"而不是"修改已有清空逻辑"——这种情况下只需要跑新增的专属测试文件，加上**全仓库 `grep` 直接触碰到的函数名**（如 `newGame(`）在 `testclass/` 里命中的全部既有测试文件（不能只跑新写的那一个测试文件，要先 `grep` 确认没有遗漏其它已经在测这个函数的既有测试）。

    **最小相关测试集的组成，不是"只跑新写的那个测试文件"**：必须是「改动新增的专属测试文件」∪「对改动触碰到的每一个已存在的函数名，在 `testclass/*.js` 里 `grep` 出的全部既有测试文件」的并集——遗漏后者等于假设"这个函数之前没有被任何测试覆盖"，这个假设本身需要先用 `grep` 验证过，不能凭印象跳过。

    **拿不准算哪一类时的默认原则**：跑全量。全量套件（`run_all_tests.js`，含 cache-bust 检查）本身运行成本是可接受的分钟级开销，而漏测一次连带影响的排查成本（尤其是 soak.js 那几轮压测反复暴露过的、跨技能/跨座位的间歇性卡死）远高于多跑几分钟测试——这条默认值本身就是从那几轮排查成本里换来的教训，不是凭空定的保守姿态。
...
- **代码已是多文件结构，但 `game.js` 已经很大**（截至 CORE-193 复核：`game.js` 约 7400 行、`bot.js` 约 5800 行、`render-controls.js` 约 3100 行、`skills.js` 约 3500 行、`render.js` 约 2400 行、`data.js` 约 1200 行）：`game.js` 比写下这条时又翻了一倍，按域进一步拆分（把武将技能/装备特效/伤害结算从 `game.js` 里拆出来）比当初更迫切。
