...
10. **分支管理**：**所有开发/提交/推送一律在 `main` 分支上进行，永远不要碰其他分支**（含 `wenwen_dev`/`chengcheng`——这条纪律 2026-08-10 由用户明确立下："反映到main，永远不要碰其他分支"；此前"wenwen_dev 自由提交、main/chengcheng 受保护"的旧纪律已作废）。**注意**：`chengcheng` 曾是 GitHub Pages 部署源（push main 不触发 Pages 构建、需把 chengcheng 快进到 main 才能触发线上更新的历史机制，见 progress-log-8 排查记录）——既然永远不碰它，线上将停留在最后一次同步的版本；若日后需要上线，由用户另行指示（改 Pages source 配置为 main，或明确说同步 chengcheng）。**每次开始新任务前，先用 `git branch` 确认当前在 `main` 分支上**。
