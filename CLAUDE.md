...
10. **分支管理**：**默认开发分支是 `wenwen_dev`——任何改动自由提交/推送 `wenwen_dev`，不需要用户批准**；**`main` 与 `chengcheng`（GitHub Pages 部署源）是受保护分支，只有用户明确说"合并 main"时才允许合并/推送，其余时候绝对不碰**（这条纪律 2026-08-09 由用户明确立下："我不说合并main绝对不可以合并，仅可以自由提交wenwen"）。合并 main 的常规流程：在 `wenwen_dev` 上开发验证 → 用户确认后合并进 `main` → 再把 `chengcheng` 快进到 `main`（Pages 构建触发线上更新）。**每次开始新任务前，先用 `git branch` 确认当前分支**，默认应该已经在 `wenwen_dev` 上。
