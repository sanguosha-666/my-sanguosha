...
['config.js','data.js','room-lifecycle.js','game.js','weapons.js','skills.js','bot-ai-bus.js'].forEach(f=>vm.runInContext(fs.readFileSync(f,'utf8'),sandbox,{filename:f}));
...
assert(g.pending.publicUntil-g.pending.askedAt>=1000);
assert.strictEqual(g.players[0].hand.length,0,'公共窗口结束前锦囊不得立即结算');
assert.strictEqual(R('maybeAutoRespondTimeout(__g)'),false,'未满1秒不得自动结束');
