...
## 武圣(关羽红牌→杀)候选真空修复——排除条件自相矛盾,和龙胆一起彻底收尾这条排查线

- **背景**：上次系统性排查确认候选真空这类bug只会发生在武圣/龙胆两个技能上，龙胆已修（见上一条记录）。本次修武圣：`isWushengShaCard`原有的排除条件`resolveActionId(g,me,c)!=='杀'`，设计假设是"resolveActionId已经解析成杀，说明常规枚举那边肯定已经收录了"——但常规枚举`enumerateAllLegalOneStepActions`用的是不认转化能力的哑函数`botActionId`（只看牌名本身），`resolveActionId`是给人类点击弹窗用的智能解析函数，两者对同一张牌的判断根本不是一回事。`resolveActionId`解析成`'杀'`正好精确对应"这张牌自己完全没有可用路径"的两类真空场景（①闪：没有`CARD_PLAYS['闪']`入口，`ownSpec`检查永远失败，恒定解析成`'杀'`；②红色`target:false`牌自己此刻打不出，如满血的桃），而这两类恰恰是常规枚举完全不收录的——于是排除条件精确排掉了本该由这个函数兜底的真空区间，变成"两头都以为对方已经覆盖，实际上谁都没覆盖"。
- **修复**：去掉`resolveActionId!=='杀'`这条排除条件。真实dump验证（Playwright真实浏览器）：满血关羽只有一张闪时，`resolveActionId`===`'杀'`、`isWushengShaCard`从false变true、`seatPickMatch`从false变true、正确生成"武圣→target"候选，`execute`真实打出（闪进弃牌堆、手牌清空、`g.phase`推进到`'respond'`）；满血+只有一张桃时同样从false修复为true（常规枚举里桃自己是空的，`normalCandidatesCount===0`，靠武圣候选兜底）；未满血+桃的回归场景确认不受影响——`resolveActionId`本来就解析成`'桃'`（own use优先），常规枚举正常收录"出桃"这一条，这条路径在旧代码里`resolveActionId!=='杀'`本来就恒真、从未被排除过，去掉这条判断对它没有任何影响。
- **真实dump过程中额外揪出并修复的两个latent bug（同一次一并处理，不是范围外的顺手重构，是本次改动直接暴露、且和本次改动同一批测试即可覆盖的真实缺陷）**：
  1. **去掉排除条件的副作用之一**：一张红色的**真杀**（`card.name`本身就是`'杀'`）现在也会满足`canUseAs(...,'杀')`——`canUseAs`第一行`isShaName(card.name)`对真杀恒真，和任何cap无关。真杀不是"转化"，常规枚举本来就会按它自己的名字正确收录一条"出杀"候选，武圣不该再重复注册一条——旧的`resolveActionId!=='杀'`条件其实同时兼顾了两件不相关的事（错的：把闪/满血桃这些真空一并滤掉；对的：把真杀本身滤掉不重复注册），去掉整条判断把"对的"那一半也丢了。真实dump复现：红杀满血场景下`isWushengShaCard`/`seatPickMatch`都变成了`true`，会产生和常规枚举重复的候选。**修复**：改成显式判断`card.name!=='杀'`（这张牌本身不是杀，才谈得上"转化"），精确只保留"不重复注册真杀"这一个意图，不影响真空的修复效果。
  2. **武圣/龙胆的颜色判断与名字判断互相污染**：`isWushengShaCard`/`isLongdanShaCard`（上次任务新增）都直接查`canUseAs(me,c,'杀')`的总返回值，但`canUseAs`里武圣（查颜色，不看名字）和龙胆（查名字，不看颜色）是两条独立分支——一个只有武圣、没有龙胆的关羽拿到红色闪时，`canUseAs`因为武圣分支返回true，但`isLongdanShaCard`只检查`card.name==='闪'`+`canUseAs(...)`为真就判定命中龙胆，完全没检查这张牌命中`canUseAs`是不是真的经过了龙胆那条分支——于是被错误标记成"命中龙胆"（反过来，赵云拿红闪也会被错误标记成"命中武圣"）。真实dump验证：关羽+红闪场景下`BOT_SEAT_PICKS.longdan.match`本应`false`却返回`true`；赵云+红闪场景下`BOT_SEAT_PICKS.wusheng.match`本应`false`却返回`true`。功能上不会真正出错（`playCard`执行时走的是真实`canUseAs`判断，游戏状态不会错），但会产生技能来源打标错误、且和真正命中的那个技能产生重复候选（AI候选列表里同时出现"武圣→X"和"龙胆→X"两条本质相同的选项）。**修复**：`isWushengShaCard`显式加`hasCap(me,'wusheng')`、`isLongdanShaCard`显式加`hasCap(me,'longdan')`，不再依赖`canUseAs`总返回值隐含"一定是通过对应那条分支"这个不成立的假设。
- **测试**：真实dump（Playwright真实浏览器+`tx`直接注入状态）覆盖：①核心场景（满血+仅一张闪，`resolveActionId`/`isWushengShaCard`/`seatPickMatch`/候选内容/`execute`真实打出全部验证）；②满血+仅一张桃（同上一套断言）；③未满血+桃回归（常规枚举正常收录"桃"，`isWushengShaCard`不受影响仍为true——确认这是改动前就有的既有行为，不是本次引入）；④红杀本身不应产生重复候选（发现即修复的latent bug 1）；⑤武圣/龙胆颜色-名字交叉污染三态验证：关羽+红闪只命中武圣、赵云+红闪只命中龙胆、赵云+黑闪只命中龙胆不命中武圣（发现即修复的latent bug 2）。`run_ai_bus_l3_test.js`新增4项断言（真空修复核心场景+回归场景+颜色名字交叉污染验证，206项，203→206），既有"红杀本身不应命中武圣"断言注释同步更新为新的判断依据（`card.name==='杀'`而非`resolveActionId`）。AI-bus全部10套件+仓库既有6套件与基线完全一致（`fazheng`8/3、`cixiong`17/3两个既有不相关基线未变；`identity_mode`出现过一次34/1的瞬时抖动，连续重跑5次均35/0，确认是既有的测试自身flaky、与本次改动无关，不是新增回归），14个核心文件`node --check`全过。
- **改动范围**：`bot.js`（`isWushengShaCard`去掉`resolveActionId!=='杀'`排除条件、改用`card.name!=='杀'`精确排除真杀本身、新增`hasCap(me,'wusheng')`显式限定；`isLongdanShaCard`新增`hasCap(me,'longdan')`显式限定）、`run_ai_bus_l3_test.js`（新增4项断言+既有断言注释更新）、`index.html`（`?v=318→319` ×14）。
- **转化技能候选真空系列彻底收尾**：武圣（本次修复）+龙胆（上次修复）是这条排查线找到的完整、唯一的两个候选真空实例，均已修复并通过真实dump验证。这条bug线到此完整闭环——不是口头声明，两次修复过程中都专门做了结构性证明（`resolveActionId`只检查`'杀'`这一个role）排除了第三个实例存在的可能性。顺带发现的连环（庞统，机器人零入口）、急救（华佗，候选省略，代码已有注释承认）仍是不同类型的缺口，保留在待处理清单里，不受本次影响。

## 于吉蛊惑声明借刀杀人——从最初排查搁置到本次完整实现(服务端+客户端)

- **这条缺口的完整来龙去脉**：这是本对话线里最早排查确认、当时刻意搁置的一个已知限制——`guhuoClaimableNames()`把"借刀杀人"排除在蛊惑可声明范围之外，原因不是笔误：蛊惑现有的目标选择流程（`guhuoChooseTarget`/`finishGuhuo`）只支持单目标声明，而借刀杀人是双目标机制（先选A有武器的角色，再选B在A攻击范围内的角色），接不上现有的单目标流程；`CARD_PLAYS['借刀杀人'].effect`当时特意留成空函数（注释写"正常流程不会走到这里"）。本次任务：核实"朋友已改好UI"这个前提（结论：在`chengcheng_dev`/`main`/`bobo_dev`三个分支和本机`bk_sanguosha`备份目录里均未找到任何相关代码，用户确认"改动其实还没做"），随后从零给出完整设计方案并实现。
- **核心设计（关键结构性发现，决定了整个方案的走向）**：`finishGuhuo`（skills.js）在`spec.target`为真时，会自动把状态设成`{type:'guhuoTarget', sourceSeat, actualCard, claimedCard}`+`phase='guhuoTarget'`——`CARD_PLAYS['借刀杀人']`早就是`target:true`，这一步对借刀杀人**完全免费复用，不需要新增pending类型**。也就是说声明→质疑→（若通过）进入目标选择，这条"先质疑后结算"的顺序是现有架构（`finishGuhuo`只在`resolveGuhuoAfterQuestion`质疑轮结束后才被调用）天然保证的，不需要专门设计。
- **实现**：
  1. `skills.js`：`guhuoClaimableNames()`去掉"借刀杀人"排除——放开后它会正常出现在`Object.keys(CARD_PLAYS)`生成的候选名单里。
  2. `skills.js`新增`guhuoChooseJiedaoTarget(seatA, seatB)`：复用现有`guhuoTarget`这个pending（不新增类型），校验逻辑照抄`jieDaoShaRen`（A有武器/不是自己/存活，B在A攻击范围内/不是A/存活/空城判空）+`isZhichiImmune`/`weimu`同款检查（和`guhuoChooseTarget`里已有的检查对齐），通过后`discard.push(d.actualCard)`+`startTrick(g,{trick:'借刀杀人', from:sourceSeat, to:seatA, seatB})`——之后完全复用借刀杀人已有的无懈窗口/`jiedaoChoice`流程，不用碰。
  3. `guhuoChooseTarget`加一行防御性拒绝：`if(d.claimedCard.name==='借刀杀人') return g;`——防止客户端调错函数、把扣置牌通过空`effect`静默吞掉。
  4. `render.js`：座位点击块（原1229-1243行附近）新增`isGuhuoJiedaoSel`分支，mirror既有`isJiedaoSel`/`jiedaoSeatA`两步状态机（选A→选B→`confirmAndPlay`提交），用独立的新客户端变量`guhuoJiedaoSeatA`（不复用`jiedaoSeatA`本身——两者触发的`g.phase`不同，play vs guhuoTarget，理论上不会同时激活，但分开命名更清楚地表明是两条独立状态机），最终调`guhuoChooseJiedaoTarget`而不是`guhuoChooseTarget`。同时在`render()`清理块、"结束出牌"按钮的cleanup列表里补上`resetGuhuoJiedao()`，跟随既有`resetJiedao()`同款兜底写法（离开对应阶段自动清空，不留残留）。
  5. `render-controls.js`：`guhuoTarget`的banner文案按`claimedCard.name==='借刀杀人'`分支，声明者视角显示"点一名持有武器的角色作为目标A"/"已选中XX为目标A，点A攻击范围内的另一名角色作为目标B"，旁观者视角显示"正在为【蛊惑】声明的【借刀杀人】选择目标…"。
- **AI决策侧的核实结论（本次唯一的"确认不需要改"项）**：`guhuoQuestion`的AI决策（`buildBotGuhuoVisibleState`）只把`claimedCardName`当一个不透明字符串塞进prompt，通读完整实现确认没有任何按牌名分支的硬编码逻辑——**这次接入不需要碰它，AI已经能通用处理任意声明名，包括"借刀杀人"这个新增值**。另外确认机器人从来不会主动发动蛊惑本身（`startGuhuo`在`bot.js`里零引用），这是既有、和这次改动完全无关的缺口，不受影响也不在本次范围内。
- **真实dump验证（Playwright真实浏览器+`tx`直接注入状态，覆盖全部关键分支）**：①完整正常路径——声明→两个bot都不质疑→选A(有武器)/B(在A范围内)→（无人有无懈可击，自动跳过wuxie窗口）→进入`jiedaoChoice`→A选择弃武器，武器正确转入使用者（于吉）手牌（不是discard，符合官方规则）、`g.phase`回到`'play'`；②防御性拒绝——直接调`guhuoChooseTarget`（故意调错函数）处理一个声明为借刀杀人的pending，确认被拒绝、pending原地不动、扣置牌没有被吞（discard里没有这张牌）；③声明为假（实际是杀，谎称借刀杀人）被质疑→正确按蛊惑既有的"假声明"逻辑作废进弃牌堆，不产生任何`guhuoTarget`选择；④声明为真（实际就是借刀杀人）被质疑→验明为真，质疑者正确获得【缠怨】，正确接回`guhuoTarget`选择流程（不是被吞掉）；⑤`buildBotGuhuoVisibleState`对`claimedCardName='借刀杀人'`正常工作，构造出的AI视图不泄露`actualCard`，`guhuoQuestionMatch`正确命中；⑥补充场景——A选择用杀而不是弃武器时，B正确受到伤害（无闪可挡）；⑦补充场景——目标区域内有人持有【无懈可击】时，能正确抵消蛊惑声明的借刀杀人（`startTrick`创建的`wuxie`pending字段`trick:'借刀杀人'`/`to`/`seatB`齐全，和`resolveTrick`原有的借刀杀人处理完全对齐）；⑧回归——普通借刀杀人（`jieDaoShaRen`手牌直出，不经蛊惑）流程完全不受影响，逐字复现改动前行为。
- **测试**：全部走真实dump（Playwright headless Chromium+真实Firebase），未新增vm沙箱持久化测试文件——核实过项目里原始借刀杀人机制本身也没有对应的持久化vm测试文件（只在当时用真实dump验证过），本次遵循同一惯例。既有回归：AI-bus全部10套件（`c_window`34/`core`10/`info`17/`l1`26/`l2`24/`l3`206/`lordskill`45/`model_picker`17/`summary`13/`timeout`8，全部0失败）+仓库既有6套件与基线完全一致（`fazheng`8/3、`cixiong`17/3两个既有不相关基线未变），14个核心文件`node --check`全过。
- **顺带发现、未处理的既有缺口（如实记录，不在本次范围内）**：`cancelGuhuoTarget()`（skills.js）从一开始就没有被任何UI按钮调用过——单目标声明流程（普通的蛊惑声明其它基本牌/锦囊）目前也没有"取消"按钮，这是蛊惑目标选择阶段本身早就存在的、和这次借刀杀人接入无关的缺口，本次没有顺手修。
- **改动范围**：`skills.js`（`guhuoClaimableNames`去掉排除+新增`guhuoChooseJiedaoTarget`+`guhuoChooseTarget`防御性拒绝）、`render.js`（座位点击块新分支+清理列表补充）、`render-controls.js`（新客户端状态变量+banner文案分支）、`index.html`（`?v=319→320` ×14）。

## 修复机器人出杀顺序错误——先出杀再喝酒,酒的伤害加成从来没被真正吃到

- **背景**：用户反馈机器人每次都先出杀再喝酒,而【酒】的效果是"本回合下一张【杀】造成的伤害+1"——加成必须先喝酒再出杀才生效,顺序反了杀已经结算完,加成完全浪费。用户要求先真实dump定位断点(区分本地启发式/AI决策/决策循环结构三个可能原因),再给方案。
- **排查结论(逐条对应用户的三个问题)**：
  1. **本地启发式确实存在这个顺序问题**：`botCardPriority`(bot.js:470-482)是一张不带上下文的静态牌价值表，`杀`(`isShaName`分支)=66，`酒`=40——纯粹的固定优先级排列，完全没有意识到这两张牌之间存在"必须先用酒才能让杀吃到加成"的顺序依赖。真实dump验证：`杀`候选分数≈72~76(66+目标分)，`酒`候选恒为40，`localFallbackPlayWindow`(无密钥兜底，取最高分候选)永远先选杀。
  2. **AI侧prompt完全没提过这条规则**：`buildBotPlaySystemPrompt`拼接的`BOT_STRATEGY_GUIDANCE_PLAY`通读全文，只有三条通用价值判断格言（"损人利己/损己利人"、试探消耗、五谷无懈别乱用），没有任何一句提到酒和杀的顺序依赖——这是一处遗漏，不是"AI应该能推理出来"，因为AI只能看到候选列表里两个独立的选项+一个不代表最优解的`localHeuristicScore`参考分，没有渠道知道"这两张牌之间存在具体的游戏机制关联"这种细节规则。
  3. **决策循环结构是"逐次单张,循环"，不是"一次性多张规划"**：出牌阶段的实际执行路径是`runBotActionWindow`（"弱C"/"强C"框架，Milestone C1起）——有密钥时循环（execute→等提交快照→重枚举→再选，直到结束/8步上限/窗口失效），每一步都会拿到刚提交完的最新状态重新枚举候选；无密钥时每次调用只执行`localFallbackPlayWindow`选出的单张最高分候选，然后返回，等下一次调度周期再问一次。这意味着**不需要引入"一次性规划整个回合牌序"这种更复杂的机制**——只要保证"酒和杀同时是本次候选时,酒的分数排在杀前面"这一件事,单次决策的排序修正就足够让多步循环自然地先出酒、再在下一步（有酒的循环内)或下一次调度（无密钥单步）出带加成的杀。
- **修复**：
  1. `bot.js`的`enumerateAllLegalOneStepActions`（Milestone C0/C1候选枚举核心）在构建完候选数组后、排序截断之前，新增一段针对性修正：若候选里同时存在`酒`和`杀`，把`酒`候选的`localHeuristicScore`拉到比场上所有`杀`候选的最高分还高一点。**刻意不改动`botCardPriority`这张通用表本身**——这张表还被弃牌优先级排序复用（"酒排在杀前面弃"在那个场景本身没错），全局改表会污染弃牌逻辑；这次的修正只作用于"这一次出牌候选列表"这个局部场景，酒/杀各自单独出现时的评分完全不受影响（已有回归测试锁定）。
  2. `bot.js`的`BOT_STRATEGY_GUIDANCE_PLAY`新增一句硬机制事实提醒："【酒】的效果是本回合下一张【杀】造成的伤害+1，这个加成必须先使用【酒】、再使用【杀】才会生效……如果候选列表里本回合同时有酒和杀可以出，应该先选酒"——写法上和`BOT_STRATEGY_GUIDANCE_TARGET`里"黑杀对仁王盾无效"那条硬事实提醒同一先例（即使是硬事实，也放进同一份"决策参考"文本里，不单独开一段规则列表）；分数修正和文字提醒是双重保险,`localHeuristicScore`只是参考分、AI仍可能选别的候选，但至少不会因为"根本不知道这条规则"而选错顺序。
- **真实dump验证（Playwright真实浏览器+`tx`直接注入状态）**：①本地兜底场景——手里同时有杀和酒，`enumerateAllLegalOneStepActions`显示酒分数(76.59)高于杀(75.59)，`candidatesOrder`排序为`['酒','杀','杀']`，`localFallbackPlayWindow`正确选中酒；②回归——只有酒/只有杀时分数不受影响（40 / 72.27，和改动前一致）；③端到端两步执行——第一次调用`runBotActionWindow`正确打出酒（`jiuUsed:true`、`jiuShaBonus:true`），第二次调用正确打出杀并带上伤害加成，目标hp从4掉到2（1点基础伤害+1点酒加成，`jiuShaBonusAfter:false`确认加成被正确消耗）——这是完整还原真实游戏两次决策周期的端到端验证，不是只看分数排序。
- **测试**：`run_ai_bus_l3_test.js`新增2项断言（顺序修复核心场景+回归场景，208项，206→208）。排查过程中发现`mkSeatG`测试helper给所有座位默认填了`role:'zhu'`（单纯图方便，不是身份局测试场景），这个默认值会让`botTargetScore`走身份局的嫌疑度判断分支（`suspicion<30`直接返回`-Infinity`），导致新测试如果不清空`role`会测出一个不代表真实ffa场景的`-Infinity`分数——新增了`clearRoles`辅助函数清空座位角色，确保测的是酒/杀本身的相对分数，不是被一个和本次改动无关的测试默认值污染出的假结果。AI-bus全部10套件（`c_window`34/`core`10/`info`17/`l1`26/`l2`24/`l3`208/`lordskill`45/`model_picker`17/`summary`13/`timeout`8，全部0失败）+仓库既有6套件与基线完全一致（`fazheng`8/3、`cixiong`17/3两个既有不相关基线未变），14个核心文件`node --check`全过。
- **改动范围**：`bot.js`（`enumerateAllLegalOneStepActions`新增酒→杀顺序修正+`BOT_STRATEGY_GUIDANCE_PLAY`新增规则提醒）、`run_ai_bus_l3_test.js`（新增2项断言）、`index.html`（`?v=320→321` ×14）。

## 修复aiSummary跨座位异步竟态——迟到的updateAiSummary响应会覆盖后来座位的归属

- **背景**：这是审查一个非本对话线设计的功能（AI摘要记忆机制`aiSummary`，commit `109f8a8`独立加入）时顺带发现的问题，不属于隐藏信息泄露（输入素材复用已有的严格脱敏`buildBotVisibleState`，AI没有渠道看到真实隐藏值），但是一个真实的正确性bug：`updateAiSummary(g,seat)`是fire-and-forget调用（不await），调用方（`scheduleBotTurn`）发起请求前会先同步把`aiSummarySeat`设成目标座位，但等待真实AI网络响应期间（可能几百毫秒到几秒），`aiSummarySeat`这个归属完全可能被别的座位抢走——多机器人共用一个浏览器时，`scheduleBotTurn`每次渲染都会跑，行动座位换得很频繁。resolve之后原代码无条件执行`aiSummary=text; aiSummarySeat=seat`（`seat`是出发时刻捕获的旧值），没有校验"此刻的归属是否还是出发时的这个座位"，于是把刚刚正确建立的新座位归属强行覆盖回旧座位，新座位刚攒的记忆被撕掉。
- **修复**：`bot-ai-bus.js`的`updateAiSummary`在AI响应`resolve`之后、真正写入`aiSummary`/`aiSummarySeat`之前，加一道校验：`if(aiSummarySeat !== seat) return;`——如果此刻的归属已经不是出发时的`seat`，说明这次响应已经过期（归属已经易主），直接丢弃不写入，旧摘要/新归属原样保留不受影响。写法和`botDecisionInFlight`那次同类竟态修复同一思路（异步操作完成时，判断结果是否还适用于当前状态），没有发明新模式。
- **真实场景验证**：用可控的deferred Promise模拟`callAI`的真实网络延迟——座位0发起`updateAiSummary`请求（不await）→ 请求还未resolve期间模拟座位1抢占归属（`aiSummaryReset()`+`aiSummarySeat=1`+写入座位1自己的摘要，和`scheduleBotTurn`/`callAiChooseIndex`真实会做的事一致）→ 座位0的迟到响应此时才resolve → 断言座位1的归属和摘要内容都未被覆盖。
- **测试**：`run_ai_summary_test.js`新增2项断言（`R1`竟态复现+`R2`无竟态回归），插入位置有讲究——必须放在既有第8项和第9项之间：第9/13项会把`updateAiSummary`整体替换成spy且从不恢复，如果新测试放在它们之后，实际测的就不是真实实现了（既有测试文件本身这个写法上的隐性耦合，顺手记录在注释里，不是这次引入的新问题）。既有第7项（超长摘要截断）测试因为直接调用`updateAiSummary(g,0)`、没有像真实调用方那样先同步设置`aiSummarySeat=0`，被本次修复的新校验拦下导致回归失败——补充了这行前置赋值，注释说明这不是绕过竟态防护、是补全测试没有模拟到的前置条件（真实生产代码里`scheduleBotTurn`总是先同步赋值再发起异步调用）。AI-bus全部10套件（`summary`13→15项，全部0失败）+仓库既有6套件与基线完全一致（`fazheng`8/3、`cixiong`17/3两个既有不相关基线未变），14个核心文件`node --check`全过。
- **改动范围**：`bot-ai-bus.js`（`updateAiSummary`新增归属校验）、`run_ai_summary_test.js`（新增2项断言+第7项补前置赋值+顶部计数注释14→16）、`index.html`（`?v=321→322` ×14）。

## botTwoStepA四技能自我触发机制——从"神速那次的错误排查方向"到"完全无关的真正根因"完整时间线

- **完整时间线（如实记录排查过程，不只是最终结论）**：①最初排查左慈机器人卡死，撞见的是夏侯渊【神速】，误以为根因是"摸牌阶段结束后的衔接点"这一类draw-phase-end hook缺注册（`shensuChoose1`/`shensuChoose2`确实是真实存在的调度盲区，但那次是"慢但自愈"，不是永久卡死，事后证明是排查方向被这次撞见的具体案例带偏了）；②用户带着新截图（貂蝉【离间】同样卡死）反馈回来，真实dump核实后发现**离间根本没有draw-phase-end相关的钩子**——离间是纯粹的出牌阶段主动技能，和神速完全不是同一类机制，"摸牌阶段结束衔接点"这个假设对离间不成立；③追着"离间为什么会卡在play阶段"这条线往下查，真实dump精确复现`botSeatForState`正确解析出座位、`botDecisionInFlight`不忙、但20秒零推进——最终定位到`lijianTwoStep`（以及借刀杀人/丈八蛇矛/仁德）共享的"客户端本地两步/三步状态机"架构本身：阶段A/B的`execute`只把选择存进`botTwoStepA`这个纯客户端变量，完全不写Firebase，而机器人调度唯一的触发来源是Firebase的onValue事件——没有写入就没有下一次调度，在没有其他人/事同时触发新写入的"安静房间"里永久卡死；④用原生貂蝉（不借用）复现同样的永久卡死，证实这**和左慈/化身机制完全无关**，是四个技能共享的架构缺陷，左慈只是恰好两次都撞见了不同的、各自独立的技能问题。
- **修复**：`scheduleBotTurn`的`setTimeout`回调`finally`块里，`botDecisionInFlight`清零之后，新增`else if(botTwoStepA)`分支，复用`botMissedSchedule`同一个`scheduleBotTurn(currentG)`入口主动补一次调度——同步调用，不用`setTimeout(fn,0)`（理由和`botMissedSchedule`那次论证完全一致：`scheduleBotTurn`本身极轻，真正的重活`runBotDecision`必然要再等一次650~1150ms的debounce才会执行，不存在同步递归风险）。用`else if`（不是独立`if`）避免和`botMissedSchedule`同帧内重复调用两次（两个分支最终都是"拿最新状态重新走一遍完整判断"，命中一次就够）。**一处改动，集中在`scheduleBotTurn`一个函数内部，自动覆盖借刀杀人/离间/丈八蛇矛/仁德全部四个技能**（以及未来任何新增的`botTwoStepA`用法）——不需要touch各自的`execute`函数：丈八蛇矛有两个本地-only中间阶段（A→B、B→C）也不需要特殊处理，因为每次dispatch结束都会经过这同一个`finally`，两次转换各自触发各自的下一次重查。
- **真实dump验证（Playwright真实浏览器）**：①差分对照——现在的`bot.js`（有修复）vs 用`git stash`临时还原到修复前的`bot.js`，同一个借刀杀人"安静房间"场景，修复前20秒零推进（`resolved:false`，`botTwoStepA`卡在`{decisionId:'jiedaoTwoStep',a:2}`），修复后1~3次650~1150ms debounce内自主完成（`resolved:true`）；②四个技能逐一构造"安静房间"场景（离间-原生貂蝉/借刀杀人/丈八蛇矛-两阶段/仁德），全部在数秒内自主完成，不需要任何外部render触发；③压力测试——5轮×4个技能=20个独立"安静房间"场景全部通过，0失败。
- **测试**：`run_ai_bus_l3_test.js`新增2项断言（借刀杀人两阶段自我触发+丈八蛇矛三阶段/两次中间转换自我触发，均用真实(未stub)的`setTimeout`跑完整的debounce周期，210项，208→210）。排查中发现这个测试沙箱顶层只有`window.setTimeout`（转发到真实Node setTimeout），没有裸`setTimeout`——需要在测试里显式`setTimeout=window.setTimeout;clearTimeout=window.clearTimeout;`补一个裸全局绑定，否则`scheduleBotTurn`内部调用会直接抛`ReferenceError`（这不是bug，是这套vm沙箱本来的设计，其它测试靠这个特性反过来临时stub掉`setTimeout`，这次是反过来接上真实定时器）。差分验证过（临时`git checkout bot.js`还原到修复前）这2项新断言在修复前确实会失败（`实际 []`，callback从未被调用），不是从一开始就通过的空断言。AI-bus全部10套件（`c_window`34/`core`10/`info`17/`l1`26/`l2`24/`l3`210/`lordskill`45/`model_picker`17/`summary`15/`timeout`8，全部0失败）+仓库既有6套件与基线完全一致（`fazheng`8/3、`cixiong`17/3两个既有不相关基线未变），14个核心文件`node --check`全过。
- **已知的正交风险（如实记录，不在本次修复范围内）**：如果`botTwoStepA`挂起期间外部条件突变（比如玩家死亡、技能能力被剥夺），导致原技能的`match`不再满足，`botTwoStepA`会一直挂着直到被某个不相关的`resetBotTwoStep()`调用清掉——这个风险在修复前后都存在（修复本身不会让它更差，自我触发反而减少了对"随机外部事件"的依赖，但没有专门解决"挂起状态和游戏状态不同步"这个更根本的问题），需要专门排查`dealDamage`/死亡判定和`botTwoStepA`挂起状态之间的交互才能彻底解决，这次不处理。
- **改动范围**：`bot.js`（`scheduleBotTurn`的`finally`块新增`else if(botTwoStepA)`自我触发分支）、`run_ai_bus_l3_test.js`（新增2项断言）、`index.html`（`?v=322→323` ×14）。
- **修复张角【鬼道】guiduAsk normalize 误杀（Firebase 吞空数组的 `askedSeats` 字段）**：debugLogs 抓到一条真实 `pending_orphan_detected`（reason 为 `A:normalize校验未通过,pending结构不合法(guiduAsk)`），pending 快照里 `sourceSeat:0`/`judgedSeat:0`/`judgeCard`/`resume` 全部合法，唯独**没有 `askedSeats` 字段**——而 normalize 校验里有 `!Array.isArray(d.askedSeats)` 这一条。**根因**：`maybeGuidu` 创建 guiduAsk pending 时 `askedSeats: []` 是空数组，Firebase Realtime Database 不保留空数组（这是本项目反复踩过的已知坑，CLAUDE.md 有专条），同步回来后该字段直接缺失（undefined），normalize 把它当成"结构不合法"的脏数据整体清空 pending——真实联机局中张角鬼道询问在建立后立刻消失，且本地 vm 沙箱测试（内存 stub 不模拟 Firebase 序列化）永远测不出来（同"Firebase 不保存空数组"一系的老问题）。**修复**：normalize 里 guiduAsk 校验块改为先补默认值 `if(!Array.isArray(d.askedSeats)) d.askedSeats=[];` 再校验真正结构性的字段（sourceSeat/judgedSeat 合法性、judgeCard/resume 存在性）——与项目内已修过的同类问题完全同一套写法：`xuanfengPick` 的 targets/discardedCounts（game.js 约1207行注释明确写"Firebase 不保留空数组…合法初始状态,不能当成脏数据清掉 pending"）、`guhuoQuestion` 的 questioners/answered。座位号越界等真脏数据仍拦截（校验不放水）。**同类排查**：逐一核对 normalize 里全部 `Array.isArray(d.*)` 校验点，确认只有 guiduAsk.askedSeats 是"创建点可能传空数组"的（luanwuChoose.remainingSeats 有 `Array.isArray(r.remainingSeats) ? slice() : []` 的兜底创建、且乱武链创建即消费风险窗口极小；lirangRecover.cards/luoyingAsk.cardIds/enyuanChooseOption.heartCards 创建前都有 `length===0 return` 守卫，非空保证），未发现第二处同类缺陷。**测试**：新增 `run_guidufix_probe.js`（vm 沙箱加载真实源码）3 个场景——①askedSeats 缺失（模拟 Firebase 吞空数组后的真实读回状态）时 pending 应保留、phase 保持 guiduAsk（修复前此场景失败：pending 被清空、phase 变 play，与日志症状一致；修复后通过）；②askedSeats 是非空数组不受影响；③sourceSeat 越界仍应判非法（校验不放水的对照）。回归：`run_guidu_nested_tx_fix_test.js` 3/3、`run_lidian_test.js` 9/9、`run_yijiask_bot_test.js` 10/10、`run_ai_bus_core_test.js` 10/10、`run_ai_bus_l3_test.js` 223/223 全过。`run_identity_mode_test.js` 有 1 条既有失败（"全员选完:主公+1血且从主公起手 - Cannot read properties of undefined"），git stash 差分验证确认**改动前就存在**、与本次无关。`index.html`: `?v=336→337` ×15。
- **AI测试托管按钮（🤖 持续托管+信息窗，Task 1-6）**：给朋友局联机调试加一个"让 AI 托管当前玩家"的按钮——点大厅 🤖 后当前浏览器座位（真人）被 AI 持续接管：轮到它时自动出牌/响应，信息窗逐条展示每次决策「①AI获取的信息 ②AI返回的信息 ③理由」，可折叠展开、可拖动、可调整大小；再点 🤖 关闭托管（信息窗保留）。**复用机制**：不新建调度链，直接在既有 bot 决策链上做"覆盖"——`botSeatForState` 的 `isBotSeat` 覆盖为"托管座位即真"（一处改动覆盖 A/B 全部段落，托管座位在 draw/play/discard/响应类 pending 全部阶段都能被调度，托管关闭时判定恒 false 零变化）；`runBotDecision` 入口放行 `isAutopilot`（= 托管开启且 seat 匹配）；`scheduleBotTurn` 对非控制器浏览器放行但**只限托管座位自己**（双浏览器驱动不冲突，轮到别的 bot 座位仍 return）。**理由采集**：`callAiChooseIndex` 解析响应时用新的 `parseBotPlayAiChoiceWithReason` 从 AI 文本里剥出 `{idx, reason}`（带 reason/代码块包裹/垃圾输入全部覆盖，无 reason 回退老解析），**仅托管命中时**写模块级 `aiTestLastReason`/`aiTestLastCall{prompt,rawResponse}` 供 record 采集——未托管时两个模块级变量零触碰（回归红线）。**关键实现点**：`aiTestAutopilot{active,seat,records}` 是唯一状态定义点（ai-bot.js，`toggleAiTestAutopilot` 开关：无密钥→弹配置框不开启 / 有密钥→开启+弹窗+按钮激活 / 再点→关闭保留记录）；`aiTestDecisionHook(g,seat,info)` 是每次托管决策的采集钩子（runBotDecision 托管分支 try/catch 包裹，组装 stateInfo=buildBotVisibleState 投影/phaseLabel/summary/prompt/rawResponse/reason 追加进 records，采集失败绝不影响决策主流程）；`aiTestLastReason` 因计划原案写在函数体内会被 var 提升成局部变量、测试恒红，实现时声明放模块级顶层。**测试**：`run_ai_test_button_test.js`（vm 沙箱加载真实完整链路 11 文件）21 项——parse 4 / callAiChooseIndex 托管命中+未托管 4（reason+aiTestLastCall）/ botSeatForState 开+关 2 / runBotDecision 守卫放行 1 / scheduleBotTurn 非控制器回调放行+入口门不调度 2 / toggleAiTestAutopilot 3 / appendAiTestRecord+toggleAiTestRecord 2 / aiTestDecisionHook 直调+runBotDecision 采集 2 / **越界边界：托管座位阵亡时 runBotDecision 首行 return 不决策不采集 1**（TDD 差分验证过：临时移除首行守卫断言必红）。全部回归：ai-bus 10 套件（l1 26/l2 24/l3 223/core 10/info 17/part2 11/c_window 34/lordskill 45/summary 15/timeout 20）+ guidu 3 + lidian 全过 + yijiask 10 + 14 核心文件 `node --check` 全过，0 失败。`index.html`: `?v=337→338` ×15。
- **排查"线上看不到AI测试按钮"——根因是 GitHub Pages source 分支配置为 chengcheng 而非 main**：AI 测试托管按钮功能（`?v=338`）已合并进 `main` 并 push，但线上 `https://zjc-taikutu.github.io/my-sanguosha/` 始终显示旧版（`?v=336`、无新按钮、`last-modified` 停在 8月8日）。排查过程：①先怀疑构建延迟/失败——查 GitHub Actions，发现 8月9日以来 push main 的记录为 0 个新 run，`pages-build-deployment` workflow 完全没被触发；②push 空提交、push 真实内容改动（`docs/README.md`）均无法触发；③截图确认 Actions 页面所有构建记录的触发分支都是 **`chengcheng`** 而非 `main`——**GitHub Pages 的 source 分支配置成了 `chengcheng`**（旧快照，HEAD 停在 `8ae890b`，恰好就是线上部署的版本），而 `main` 领先 chengcheng 16 个提交。**根因：push `main` 永远不会触发 GitHub Pages 构建，因为 source 配置的是 `chengcheng`**。修复：把 `chengcheng` 快进到 `main`（纯快进，chengcheng 无独有提交，零冲突）并 push，`chengcheng` 从 `8ae890b..24cccf8`，构建立即触发（8月9日 08:43 success），线上更新为 `?v=338` 并出现 🤖 按钮。**通用教训**：①"push 了 main 但线上没变"不能只怀疑构建失败/延迟，还要确认 GitHub Pages 的 source 分支到底配置成哪个分支——用截图看 Actions 页面的触发分支，或对比线上部署版本与各远程分支 HEAD；②`chengcheng` 分支是 GitHub Pages 的部署源，任何要上线的改动除了 push `main`，还要同步 `chengcheng`（或改 Pages 配置让 source 用 `main`）。
