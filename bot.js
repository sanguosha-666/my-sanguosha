...
// 实测(2026-08,见 progress-log 对应条目):N=140(含藤甲2/白银狮子1/朱雀羽扇1),杀系(杀30+火杀5+雷杀9)K=44(31.4%),
...
// "手牌里还混着装备/锦囊"那条**是错的**——超几何本来就是从整副 140 张里抽,装备锦囊
...
// 成立。下限只对 n>=1 生效(实际从不触发,n=1 的闪已经是 0.091)。
//
...
const BOT_DODGE_TABLE = [0, 0.091, 0.173, 0.247, 0.313, 0.372, 0.425, 0.473, 0.515, 0.553, 0.587, 0.618, 0.645];
const BOT_SLASH_TABLE = [0, 0.267, 0.452, 0.579, 0.666, 0.725, 0.766, 0.794, 0.812, 0.825, 0.833, 0.839, 0.843];
...
// 用的事实",不写推测。刻意和 callAiChooseIndex 的"只输出 {"choice":数字}"约定分开,
// 因为这是"更新记忆"任务、不是从候选里选 index(CORE-140 起两者都是 JSON 输出,但
// 契约完全不同:这里是 {tactical,doctrineUpdate},那里是 {choice,reason};
// parseAiSummaryLayers 会拒绝 {"choice":N} 这类无关 JSON,不会互相误认)。
// CORE-140:改为要求两层 JSON(原先是纯文本整体覆写)。**解析失败会逐字回退到改动前的整体覆写路径**
// (见 updateAiSummary 里的说明),所以这次 prompt 变更不存在"比现状更差"的下行风险。
// 两层的分工在提示里写清楚,否则模型会把两类东西继续混在一起——这正是改动前的问题:
// 战术噪声每回合把辛苦得出的身份推断冲掉。
function buildSummaryPrompt(g, seat){
  return '你是网页版三国杀的AI机器人。请结合旧的记忆(如有)与最近发生的公开事件,'
    +'更新你的两层记忆,并且只输出一个JSON对象。'
    +'\n\n两层的分工(务必区分清楚,不要混写):'
    +'\ntactical(战术,不超过80字):**下一回合的具体打算**——首选目标是谁、'
    +'打算留住哪些牌、主要要防范什么。这一层每次都会被整体覆盖,写当前最新的打算即可。'
    +'\ndoctrineUpdate(认知增量,不超过120字):**这次新得到的、跨回合仍然成立的结论**——'
    +'比如某人身份推断的**理由**(观察到了什么行为才这么判断)、阵营关系、资源使用纪律。'
    +'这一层会被累积保留,所以只写**这次新增或需要修正**的部分,不要重复已经写过的内容;'
    +'如果这次没有新的认知,就留空字符串。'
    +'\n\n只写发生过的事实与由它推出的判断,不要编造。'
    +'\n输出格式(只输出这个JSON,不要代码块标记、不要任何解释):'
    +'{"tactical":"...","doctrineUpdate":"..."}';
}
