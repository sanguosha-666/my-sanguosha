...
Object.entries({
  huashenPick:        {skill:'化身', render:renderPendingHuashenPick},
  haoshiPick:         {skill:'好施', render:renderPendingHaoshiPick},
  jujianPickCard:     {skill:'举荐', render:renderPendingJujianPickCard},
  jujianPickTarget:   {skill:'举荐', render:renderPendingJujianPickTarget},
  jujianChooseEffect: {skill:'举荐', render:renderPendingJujianChooseEffect},
  jiangchiAsk:        {skill:'将驰', render:renderPendingJiangchiAsk},
  luoyingAsk:         {skill:'落英', render:renderPendingLuoyingAsk},
  jiushiFlipAsk:      {skill:'酒诗', render:renderPendingJiushiFlipAsk},
  jushouChoose:       {skill:'据守', render:renderPendingJushouChoose},
  qiaomengChoose:     {skill:'趫猛', render:renderPendingQiaomengChoose},
  tieqi:              {skill:'铁骑', render:renderPendingTieqi},
  shuangxiongAsk:     {skill:'双雄', render:renderPendingShuangxiongAsk},
  liegong:            {skill:'烈弓', render:renderPendingLiegong},
  qinglong:           {skill:'青龙偃月刀', render:renderPendingQinglong},
  lianyingAsk:        {skill:'连营', render:renderPendingLianyingAsk},
  guanshi:            {skill:'贯石斧', render:renderPendingGuanshi},
  shaOffsetChoice:    {skill:'杀的后续效果', render:renderPendingShaOffsetChoice},
  mengjin:            {skill:'猛进', render:renderPendingMengjin},
  yijiAsk:            {skill:'遗计', render:renderPendingYijiAsk},
  ganglieAsk:         {skill:'刚烈', render:renderPendingGanglieAsk},
  luoyiAsk:           {skill:'裸衣', render:renderPendingLuoyiAsk},
  lirangAsk:          {skill:'礼让', render:renderPendingLirangAsk},
  lirangRecover:      {skill:'礼让', render:renderPendingLirangRecover},
  zhengyi:            {skill:'争义', render:renderPendingZhengyi},
  quhuRespond:        {skill:'驱虎', render:renderPendingQuhuRespond},
  tianyiRespond:      {skill:'天义', render:renderPendingTianyiRespond},
  zhibaAsk:           {skill:'制霸', render:renderPendingZhibaAsk},
  jiemingAsk:         {skill:'节命', render:renderPendingJiemingAsk},
  xinshengAsk:        {skill:'新生', render:renderPendingXinshengAsk},
  liuli:              {skill:'流离', render:renderPendingLiuli},
  tianxiang:          {skill:'天香', render:renderPendingTianxiang},
  huashenChangeAskStart:{skill:'化身', render:renderPendingHuashenChangeAskStart},
  huashenChangePickStart:{skill:'化身', render:renderPendingHuashenChangePickStart},
  huashenChangeAskEnd:{skill:'化身', render:renderPendingHuashenChangeAskEnd},
  ...
  yaowu_choose:       {skill:'耀武', render:renderPendingYaowuChoose},
  wangxiAsk:          {skill:'忘隙', render:renderPendingWangxiAsk},
  quhuDamageChoice:   {skill:'驱虎', render:renderPendingQuhuDamageChoice},
  fanjianSuit:        {skill:'反间', render:renderPendingFanjianSuit},
  jiedaoChoice:       {skill:'借刀杀人', render:renderPendingJiedaoChoice},
  tiaoxinChoice:      {skill:'挑衅', render:renderPendingTiaoxinChoice},
  tiaoxinDiscard:     {skill:'挑衅', render:renderPendingTiaoxinDiscard},
  qiaobianTurnStart:  {skill:'巧变', render:renderPendingQiaobianTurnStart},
  qiaobianMove:       {skill:'巧变', render:renderPendingQiaobianMove},
  leijiChoose:        {skill:'雷击', render:renderPendingLeijiChoose},
  leijiJudge:         {skill:'雷击', render:renderPendingLeijiJudge},
  guiduAsk:           {skill:'鬼道', render:renderPendingGuiduAsk},
  jijiangAsk:         {skill:'激将', render:renderPendingJijiangAsk},
  hujiaAsk:           {skill:'护驾', render:renderPendingHujiaAsk},
  lieRenChoose:       {skill:'烈刃', render:renderPendingLieRenChoose},
  lieRenPickCard:     {skill:'烈刃', render:renderPendingLieRenPickCard},
  lieRenRespond:      {skill:'烈刃', render:renderPendingLieRenRespond},
  shensuChoose1:      {skill:'神速1', render:renderPendingShensuChoose1},
  shensuChoose2:      {skill:'神速2', render:renderPendingShensuChoose2},
  qiaomengPickEquip:  {skill:'趫猛', render:renderPendingQiaomengPickEquip},
  qilin:               {skill:'麒麟弓', render:renderPendingQilin},
  qiangxiChooseCost:  {skill:'强袭', render:renderPendingQiangxiChooseCost},
  qiangxiChooseWeaponFromHand:{skill:'强袭', render:renderPendingQiangxiChooseWeaponFromHand},
  qiangxiPickTarget:  {skill:'强袭', render:renderPendingQiangxiPickTarget},
  mingcePickCard:     {skill:'明策', render:renderPendingMingcePickCard},
  mingcePickTarget:   {skill:'明策', render:renderPendingMingcePickTarget},
  mingcePickTarget2:  {skill:'明策', render:renderPendingMingcePickTarget2},
  mingceChoice:       {skill:'明策', render:renderPendingMingceChoice},
  luanjiConfirm:      {skill:'乱击', render:renderPendingLuanjiConfirm},
  yijiAssign:         {skill:'遗计', render:renderPendingYijiAssign},
  luanwuChoose:       {skill:'乱武', render:renderPendingLuanwuChoose},
  hanbing:            {skill:'寒冰剑', render:renderPendingHanbing},
  zhibaGain:          {skill:'制霸', render:renderPendingZhibaGain},
  yinghunTarget:      {skill:'英魂', render:renderPendingYinghunTarget},
  yinghunChoice:      {skill:'英魂', render:renderPendingYinghunChoice},
  yinghunDiscard:     {skill:'英魂', render:renderPendingYinghunDiscard},
  enyuanChoose:       {skill:'恩怨', render:renderPendingEnyuanChoose},
  enyuanChooseOption: {skill:'恩怨', render:renderPendingEnyuanChooseOption},
  enyuanGiveCard:     {skill:'恩怨', render:renderPendingEnyuanGiveCard},
  huanhuoPick:        {skill:'眩惑', render:renderPendingHuanhuoPick},
  huanhuoPickCard:    {skill:'眩惑', render:renderPendingHuanhuoPickCard},
  huanhuoPickGotCard: {skill:'眩惑', render:renderPendingHuanhuoPickGotCard},
  huanhuoPickSecond:  {skill:'眩惑', render:renderPendingHuanhuoPickSecond},
  luanjiChoose:       {skill:'乱击', render:renderPendingLuanjiChoose},
  buquAsk:            {skill:'不屈', render:renderPendingBuquAsk},
  duanbingChoose:     {skill:'短兵', render:renderPendingDuanbingChoose}
}).forEach(function(entry){ registerStage(entry[0],entry[1]); });
function renderRegisteredPending(g,c){
  const d=g&&g.pending;
  const spec=d&&STAGE_TABLE[d.type];
  if(!spec || typeof spec.render!=='function' || g.phase!==d.type) return false;
...
function renderPendingQiaomengPickEquip(g,c){
  ...
  const p=document.createElement('p');p.textContent='选择 '+escapeHtml(target?target.name:'目标')+' 的一张装备牌：';div.appendChild(p);
  const equipDiv=document.createElement('div');equipDiv.className='equip-options';const equips=target.equips||{},names={weapon:'武器',armor:'防具',plus1:'防御马',minus1:'进攻马'};
  (g.pending.availableSlots||[]).forEach(slot=>{const card=equips[slot];if(!card)return;const b=document.createElement('button');b.className='equip-btn';b.textContent=(names[slot]||slot)+'【'+escapeHtml(card.name)+'】'+(isMountCard(card)?' (坐骑-获得)':' (弃置)');b.onclick=()=>pickQiaomengEquip(slot);equipDiv.appendChild(b);});div.appendChild(equipDiv);
  const cancel=document.createElement('button');cancel.className='cancel-btn';cancel.textContent='取消';cancel.onclick=()=>cancelQiaomeng();div.appendChild(cancel);c.appendChild(div);
}
function renderPendingQilin(g,c){
  const target=g.players[g.pending.to],labels={plus1:'+1马',minus1:'-1马'};
  if(target)['plus1','minus1'].forEach(slot=>{if(!target.equips[slot])return;const b=document.createElement('button');b.textContent='弃置'+labels[slot]+'【'+target.equips[slot].name+'】';b.onclick=()=>qilinResolve(slot);c.appendChild(b);});
  setBanner('你的【麒麟弓】发动,选择弃置 '+escapeHtml(target?target.name:'目标')+' 的哪匹坐骑。');
}
function renderPendingQiangxiChooseCost(g,c){
  const me=g.players[mySeat],div=document.createElement('div');div.className='centered',h4=document.createElement('h4');h4.textContent='【强袭】支付方式';div.appendChild(h4);
  const p=document.createElement('p');p.textContent='请选择你要支付的消耗';div.appendChild(p);
  if(me&&me.alive&&me.hp>1){const hp=document.createElement('button');hp.className='cost-btn';hp.style.background='#e74c3c';hp.textContent='失去1点体力';hp.onclick=()=>chooseQiangxiCost('hp');div.appendChild(hp);}
  if(hasWeaponToDiscard(me)){const weapon=document.createElement('button');weapon.className='cost-btn';weapon.style.background='#e74c3c';weapon.textContent='弃置一张武器牌';weapon.onclick=()=>chooseQiangxiCost('weapon');div.appendChild(weapon);}
  const cancel=document.createElement('button');cancel.className='cancel-btn';cancel.textContent='取消';cancel.onclick=()=>cancelQiangxi();div.appendChild(cancel);c.appendChild(div);setBanner('请选择【强袭】的支付方式');
}
function renderPendingQiangxiChooseWeaponFromHand(g,c){
  const me=g.players[mySeat],div=document.createElement('div');div.className='centered',h4=document.createElement('h4');h4.textContent='【强袭】选择武器牌';div.appendChild(h4);
  const p=document.createElement('p');p.textContent='请选择要弃置的武器牌';div.appendChild(p);
  (g.pending.weaponIndices||[]).forEach(idx=>{const card=me.hand[idx];if(!card)return;const b=document.createElement('button');b.className='target-btn';b.textContent='【'+escapeHtml(card.name)+'】';b.onclick=()=>chooseQiangxiWeaponFromHand(idx);div.appendChild(b);});
  const cancel=document.createElement('button');cancel.className='cancel-btn';cancel.textContent='取消';cancel.onclick=()=>cancelQiangxi();div.appendChild(cancel);c.appendChild(div);setBanner('请选择要弃置的武器牌');
}
function renderPendingQiangxiPickTarget(g,c){
  const div=document.createElement('div');div.className='centered',h4=document.createElement('h4');h4.textContent='【强袭】选择目标';div.appendChild(h4);
...
function renderControls(g){
  ...
  if(!g.started){
    ...
    if(selectedGameMode==='ffa'){
      ...
      btnPick.disabled = cnt<1; // 可点,人数不足在 onclick 拦截(与身份局同款)
      btnPick.onclick=()=>{
        const teams={};
        let hasNoTeam=false;
